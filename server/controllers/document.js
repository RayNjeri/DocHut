  ...
  list(req, res) {
    ...
    const constructQuery = (isPublic, roleId) =>
      new Promise((resolve, reject) => {
        ...
        return Role.findById(roleId).then(role => {
    ...
    if (req.roleId == 1) {
      if (req.query.limit || req.query.offset) {
        constructQuery(true, req.roleId).then(findDocs);
      } else {
        allSearch();
      }
    } else {
      if (req.query.limit || req.query.offset) {
        ...
        constructQuery(null, null, true, req.roleId).then(findDocs);
      }
    }
  },
    ...
    Document.findById(req.params.documentId)
      .then(document => {
        ...
        document
          .destroy()
          .then(() =>
            res.status(204).send({ message: 'Document Succesfully deleted' })
          )
          .catch(error => res.status(400).send(error));
      })
      .catch(error => res.status(400).send(error));
